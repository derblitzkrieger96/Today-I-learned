...
function App() {
  ...
  return (
    <>
      {/* HEADER */}
      <Header showForm={showForm} onShowFormHandler={toggleForm} />
      {showForm && (
...
function Header({ showForm, onShowFormHandler }) {
  return (
    <header className="header">
      ...
      <button className="btn btn-large btn-open" onClick={onShowFormHandler}>
...
function FactList({ facts, setFacts }) {
  if (facts.length === 0)
    return (
      <p className="message">
        No facts for this category, Try to add a new one!✌
      </p>
    );
...
function Fact({ fact, setFacts }) {
  ...
  const handleVote = async function (column) {
    setIsUpdating(true);
